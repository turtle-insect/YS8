	...
	class SaveData
	{
		...
		public bool Open(String filename, bool force)
		{
			...
			if (force == false)
			{
				...
				if (sum.sum1 == ReadNumber(0xC7FF8, 4) && sum.sum2 == ReadNumber(0xC7FFC, 4))
				{
					Backup();
					return true;
				}
		...
		public bool Save()
		{
			...
			var sum = CalcCheckSum();
			WriteNumber(0xC7FF8, 4, sum.sum1);
			WriteNumber(0xC7FFC, 4, sum.sum2);
		...
		private (UInt32 sum1, UInt32 sum2)  CalcCheckSum()
		{
			if (mBuffer == null) return (0xFF, 0xFF);
			UInt32 sum1 = 0;
			UInt32 sum2 = 0;
			for (uint i = 0; i < mBuffer.Length - 8; i += 4)
			{
				UInt32 num = ReadNumber(i, 4);
				sum1 += num;
				sum2 += ~num;
			}
			return (sum1, sum2);
